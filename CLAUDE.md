...
- **Design system** — Tailwind v4 tokens in `src/styles/global.css` `@theme`. v1 palette kept verbatim + v2 layers: saturated `deep` colors (strawberry/sky-deep/mint-deep/sunshine/grape-deep — big surfaces only: hero, buttons, badges, marquee) and `mist` tints (5 section backgrounds; max 2 per viewport, joined by Scallop/Wave dividers). Links: ALL text links get a wavy candy underline (global `a` rule in global.css;按鈕/卡片/pill 型連結例外清單在同處;勿用 `text-decoration: underline` shorthand —— 會把波浪重設成直線). Sticker grammar: `.title-pop` two-layer titles (`data-text` attr required, emoji excluded from it), `.sticker-ring` die-cut white edge, hard offset shadows, `.bg-candy-stripe`/`.bg-ruled`. **No irregular shapes**:使用者指定全站一律規則圓角方形(blob/squish/手撕角已移除,BlobFrame 實為 2rem 圓角方框). Decor components in `src/components/decor/` (Mascot, ScallopDivider, StickerBadge, PolaroidFrame, BlobFrame, DottedPath, StampSeal…). No marquees: 使用者明確不要(CP 值低、無限動畫耗效能).
