...
- **Portfolio / works** — `works` collection in `src/content.config.ts`. `/portfolio/` is a lightweight gallery of **GIF thumbnails** (`public/works/*.gif`, referenced via `withBase` in a plain `<img>` — NOT `astro:assets`, which can't keep GIFs animated). Each work's interactive `<ModelViewer>` loads **only** on its detail page `src/pages/portfolio/[id].astro` (one WebGL canvas per page). A work = a `.md` in `src/content/works/` with `model` (glb filename in `public/models/`), `thumb` (gif filename in `public/works/`), `environment`, `order`, `tags`. The rotating-GIF thumbnails are currently generated manually (a temporary `client:only` capture page spun via Playwright → frames → `ffmpeg` palette GIF); **auto-generation on push is planned** — see the friendly-authoring notes.
